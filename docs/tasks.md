...
Following Hardhat's way of working, you must specify the network against which you want to run the task using the `--network` parameter. In all the following examples, the hoodi network will be used, but you can specify any defined in your `hardhat.config` file.
...
```
npx hardhat --network hoodi_testnet deploy:all
```

When deploying to live networks like hoodi or Mainnet, please double check the environment variables:
...
Example:
npx hardhat --network hoodi_testnet upgrade:proxy --proxy-address 0x1234... --contract SSVNetworkV2 --init-function initializev2 param1 param2
```
...
npx hardhat --network hoodi_testnet update:module --module SSVOperators --attach-module true --proxy-address 0x1234...
```
...
Example:
npx hardhat --network hoodi_testnet upgrade:prepare --proxy-address 0x1234... --contract SSVNetworkViewsV2
```
