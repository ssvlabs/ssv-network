...
async function main() {
  ...
  if (rpcUrl) {
    ...
    if (targetNetwork === "hoodi") {
      process.env.HOODI_RPC_URL = rpcUrl;
    } else {
  ...
  console.log(`Preparing testnet upgrade deployments on ${targetNetwork}`);
  ...
  console.log(
    "[1/2] Deploying upgrade implementations (SSVNetworkSSVStakingUpgrade, SSVNetworkViews)"
  );
  ...
  console.log("[2/2] Deploying all module implementations");
  ...
  const { address: ssvDaoAddr } = await deployContract(ethers, "SSVDAO", [cssvToken], deployer);
  const { address: ssvViewsAddr } = await deployContract(ethers, "SSVViews", [cssvToken], deployer);
  ...
  const { address: ssvStakingAddr } = await deployContract(ethers, "SSVStaking", [cssvToken], deployer);
