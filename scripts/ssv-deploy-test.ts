...
async function main() {
  ...
  const ssvNetwork = await upgrades.deployProxy(ssvNetworkFactory, [ssvRegistry.address, process.env.SSV_TOKEN_ADDRESS, process.env.MINIMUM_BLOCKS_BEFORE_LIQUIDATION, process.env.OPERATOR_MAX_FEE_INCREASE, process.env.SET_OPERATOR_FEE_PERIOD, process.env.APPROVE_OPERATOR_FEE_PERIOD, process.env.VALIDATORS_PER_OPERATOR_LIMIT]);
  await ssvNetwork.deployed();
  console.log(`SSVToken: ${process.env.SSV_TOKEN_ADDRESS}\nSSVRegistry: ${ssvRegistry.address}\nSSVNetwork: ${ssvNetwork.address}\n`);
}
