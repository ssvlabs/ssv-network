...
describe('DAO operational Tests', () => {
  ...
  it('Get the network validators count (add/remove validaotor)', async () => {
    ...
    const register = await trackGas(
      ssvNetworkContract
        ...
        .registerValidator(helpers.DataGenerator.publicKey(1), [1, 2, 3, 4], helpers.DataGenerator.shares(4), deposit, {
          validatorCount: 0,
          networkFeeIndex: 0,
          index: 0,
          balance: 0,
  ...
  it('Get the network validators count (add/liquidate validaotor)', async () => {
    await helpers.registerOperators(0, 4, helpers.CONFIG.minimalOperatorFee);
