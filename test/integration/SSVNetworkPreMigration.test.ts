...
import { ssvNetworkFullPreUpgradeFixture } from '../setup/fixtures.ts';
import type { NetworkHelpersType, Operator, OperatorSSV } from '../common/types.ts';
import {
  CLUSTER_VERSION_ETH,
  CLUSTER_VERSION_SSV, DEFAULT_ETH_REGISTER_VALUE, DEFAULT_OPERATOR_ETH_FEE,
  DEFAULT_SHARES, EMPTY_CLUSTER,
  MAXIMUM_OPERATORS_FEE, MINIMAL_OPERATOR_ETH_FEE, MINIMAL_OPERATOR_FEE_SSV,
...
import {
  ...
  makePublicKey, registerOperators,
...
describe("SSVNetwork full integration tests with performing an upgrade on a legacy artifact", () => {
  ...
  describe("Restrictions for legacy ssv clusters", async function() {
    it("'registerValidator()' is reverted with 'IncorrectClusterVersion' if trying to register validators to a legacy cluster", async function () {
      ...
      await expect(newNetwork.connect(clusterOwner).registerValidator(
        makePublicKey(322), operatorIds, DEFAULT_SHARES, cluster)
    ...
    it("'bulkRegisterValidator()' is reverted with 'IncorrectClusterVersion' if trying to register validators to a legacy cluster", async function () {
      ...
      await expect(newNetwork.connect(clusterOwner).bulkRegisterValidator(
        [makePublicKey(322)], operatorIds, [DEFAULT_SHARES], cluster)
      ).to.be.revertedWithCustomError(newNetwork, Errors.INCORRECT_CLUSTER_VERSION);
    });
    ...
    it("'removeValidator()' is reverted with 'IncorrectClusterVersion' if trying to remove validators from a legacy cluster", async function () {
      ...
      await expect(newNetwork.connect(clusterOwner).removeValidator(
        makePublicKey(123), operatorIds, cluster)
      ).to.be.revertedWithCustomError(newNetwork, Errors.INCORRECT_CLUSTER_VERSION);
    });
    ...
    it("'bulRemoveValidator()' is reverted with 'IncorrectClusterVersion' if trying to remove validators from a legacy cluster", async function () {
      ...
      await expect(newNetwork.connect(clusterOwner).bulkRemoveValidator(
        [makePublicKey(123)], operatorIds, cluster)
      ).to.be.revertedWithCustomError(newNetwork, Errors.INCORRECT_CLUSTER_VERSION);
    });
    ...
    it("'liquidate()' is reverted with 'IncorrectClusterVersion' if trying to liquidate legacy ssv cluster with an ETH-based function", async function() {
      ...
      await expect(newNetwork.connect(clusterOwner).liquidate(
    ...
    it("'reactivate()' is reverted with 'IncorrectClusterVersion' if trying to reactivate a legacy ssv cluster", async function() {
      ...
      await expect(newNetwork.connect(clusterOwner).reactivate(
        operatorIds, cluster)
      ).to.be.revertedWithCustomError(newNetwork, Errors.INCORRECT_CLUSTER_VERSION);
    });
    ...
    it("withdraw() is reverted with 'IncorrectClusterVersion' is trying to withdraw from a legacy ssv cluster", async function() {
      ...
      await expect(newNetwork.connect(clusterOwner).withdraw(
        operatorIds, TOKEN_REGISTER_AMOUNT, cluster)
      ).to.be.revertedWithCustomError(newNetwork, Errors.INCORRECT_CLUSTER_VERSION);
    });
    ...
    it("deposit() is reverted with 'IncorrectClusterVersion' is trying to deposit to a legacy ssv cluster", async function() {
      ...
      await expect(newNetwork.connect(clusterOwner).deposit(
  ...
  describe("Function 'migrateClusterToETH'", async function() {
    it("Executes as expected and emits correct event", async function() {
      ...
      const clusterBalance =
        await newViews.getBalanceSSV(clusterOwner.address, operatorIds, cluster);
      const burnRateSSV =
        await newViews.getBurnRateSSV(clusterOwner.address, operatorIds, cluster);
      ...
      await expect(tx)
        .to.changeTokenBalances(
          connection.ethers, ssvToken,
          [newNetwork, clusterOwner],
          [-expectedClusterBalance, expectedClusterBalance]
        )
      ...
      expect(await newViews.getBalanceSSV(clusterOwner.address, operatorIds, cluster)).to.be.equal(0); // cluster already migrated, not ssv
      expect(await newViews.getBalance(clusterOwner.address, operatorIds, cluster)).to.be.equal(SMALL_ETH_REGISTER_VALUE); // cluster already migrated, balance is in eth
  ...
  describe("Legacy operators migration", async function () {
    it("Migrates operators to the eth version after migration of operator's cluster", async function() {
      ...
      for (let i = 0; i < operatorIds.length; i++) {
        ...
        expect(opSSV.validatorCount).to.be.equal(1); // 1 ssv validator added before migration
        expect(opEth.validatorCount).to.be.equal(0); // not migrated yet

        expect(opSSV.fee).to.be.equal(MINIMAL_OPERATOR_FEE_SSV);
        expect(opEth.fee).to.be.equal(MINIMAL_OPERATOR_ETH_FEE); // ssv fee is not 0
      ...
      for (let i = 0; i < operatorIds.length; i++) {
        ...
        expect(opSSV.validatorCount).to.be.equal(0); // 1 ssv validator was migrated to eth
  ...
  describe("Sanity", async function() {
    it("Migrates operators to ETH after registering an ETH validator and applies correct fees", async function() {
      ...
      // register 4 ssv operators
      const operatorIds = await registerOperatorsSSV(network, operatorOwner, 4);

      // migrate network to eth with ops having 0 validators
      const { newNetwork, newViews } = await upgradeToStakingVersion(connection, network, views);
      ...
      for (let i = 0; i < operatorIds.length; i++) {
        ...
        expect(opSSVFee).to.be.equal(MINIMAL_OPERATOR_FEE_SSV); // register value
        expect(opEthFee).to.be.equal(DEFAULT_OPERATOR_ETH_FEE); // default contract value that getter should return
      }

      await whitelistAddresses(network, operatorOwner, operatorIds, [clusterOwner.address]);

      await newNetwork.connect(clusterOwner).registerValidator(
        makePublicKey(1),
        operatorIds,
        DEFAULT_SHARES,
        EMPTY_CLUSTER,
      ...
      await networkHelpers.mine(999); // skip time to collect fees
      ...
      for (let i = 0; i < operatorIds.length; i++) {
        ...
        expect(opSSVFee).to.be.equal(MINIMAL_OPERATOR_FEE_SSV); // register value
        expect(opEthFee).to.be.equal(DEFAULT_OPERATOR_ETH_FEE); // applied to operator after the validator registration
