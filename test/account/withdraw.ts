...
describe('Withdraw Tests', () => {
  ...
  it('Withdraw from a liquidatable cluster emits "ClusterWithdrawn"', async () => {
    await utils.progressBlocks(20);
    await expect(ssvNetworkContract.connect(helpers.DB.owners[4]).withdraw(cluster1.args.operatorIds, 4000000000, cluster1.args.cluster)).to.emit(ssvNetworkContract, 'ClusterWithdrawn');
  });
