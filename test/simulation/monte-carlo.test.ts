...
(RUN_FORK ? describe : describe.skip)("Monte Carlo Upgrade Simulation", function () {
  ...
  before(async function () {
    ...
    // 6. Discover and sample mainnet operators
    console.log("[SIM] Discovering mainnet operators...");
    const currentBlock = await provider.getBlockNumber();
    const scanFrom = Math.max(0, currentBlock - 500_000); // scan last ~500k blocks
    ...
    const sampledOps = await sampleOperators(discovered, fixture.views, 20, rng);
