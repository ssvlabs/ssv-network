...
describe('Operator Remove', function () {
  beforeEach(async function () {
    ...
    await ssvNetwork.connect(account3).registerOperator('testOperator 1', operatorsPub[1], 20000)
  ...
  it('Remove operator no validators', async function () {
    ...
    await expect(ssvNetwork.connect(account2).removeOperator(operatorsIds[0]))
      .to.emit(ssvRegistry, 'OperatorRemoved').withArgs(operatorsIds[0], account2.address, operatorsPub[0])
    ...
    expect((await ssvNetwork.getOperatorById(operatorsIds[0]))[4]).to.equal(false)
  ...
  it('Remove operator with validators', async function () {
    ...
    // Check that the operator statuses
    expect((await ssvNetwork.getOperatorByPublicKey(operatorsPub[0]))[0]).to.equal('testOperator 0')
    expect((await ssvNetwork.getOperatorByPublicKey(operatorsPub[0]))[1]).to.equal(account2.address)
    expect((await ssvNetwork.getOperatorByPublicKey(operatorsPub[0]))[2]).to.equal(operatorsPub[0])
    expect((await ssvNetwork.getOperatorByPublicKey(operatorsPub[0]))[3]).to.equal('0')
    expect((await ssvNetwork.getOperatorByPublicKey(operatorsPub[0]))[4]).to.equal(false)

    // Check that user did not earn anything since operator is deleted
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal('11000000')
  ...
  it('Remove operator errors', async function () {
    ...
    await ssvNetwork.connect(account2).removeOperator(operatorsIds[0]).should.eventually.be.rejectedWith('SSVRegistry: operator deleted')
  })
})