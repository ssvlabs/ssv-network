...
describe('SSV Network Contract', function () {
  ...
  it('Operator limit', async function () {
    ...
    await ssvNetwork.connect(account3).registerOperator('testOperator 7', operatorsPub[7], 500000000);
    await ssvNetwork.connect(account3).registerOperator('testOperator 8', operatorsPub[8], 500000000);
    await ssvNetwork.connect(account3).registerOperator('testOperator 9', operatorsPub[9], 500000000);
    await ssvNetwork.connect(account3).registerOperator('testOperator 10', operatorsPub2[0], 500000000);
    await ssvNetwork.connect(account3).registerOperator('testOperator 11', operatorsPub2[1], 500000000);
    await expect(ssvNetwork.connect(account3).registerOperator('testOperator 12', operatorsPub2[2], 500000000)).to.be.revertedWith("ExceedRegisteredOperatorsByAccountLimit")
  });

  it('Operators getter', async function () {
    expect((await ssvNetwork.getOperatorById(operatorsIds[0])).map((v: any) => v.toString())).to.eql(['testOperator 0', account2.address, operatorsPub[0], '1', '100000000', '0', 'true']);
    expect((await ssvNetwork.getOperatorById(operatorsIds[1])).map((v: any) => v.toString())).to.eql(['testOperator 1', account2.address, operatorsPub[1], '1', '200000000', '0', 'true']);
    expect((await ssvNetwork.getOperatorById(operatorsIds[2])).map((v: any) => v.toString())).to.eql(['testOperator 2', account3.address, operatorsPub[2], '1', '300000000', '0', 'true']);
  });

  it('Get operator current fee', async function () {
    expect(await ssvNetwork.getOperatorFee(operatorsIds[0])).to.equal(100000000);
    expect(await ssvNetwork.getOperatorFee(operatorsIds[1])).to.equal(200000000);
    expect(await ssvNetwork.getOperatorFee(operatorsIds[2])).to.equal(300000000);
  });

  it('Balances should be correct after 100 blocks', async function () {
    await progressBlocks(100);
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9900000000000);
  ...
  it('Withdraw', async function () {
    ...
    await ssvNetwork.connect(account2).withdraw('1000000000');
    await ssvNetwork.connect(account3).withdraw('1000000000');
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9787000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(59900000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(141100000000);
  });

  it('Revert withdraw: NotEnoughBalance', async function () {
    await progressBlocks(350)
    await expect(ssvNetwork.connect(account1)
      .withdraw('80000000000000'))
      .to.be.revertedWith('NotEnoughBalance');
    await expect(ssvNetwork.connect(account2)
      .withdraw('9000000000000'))
      .to.be.revertedWith('NotEnoughBalance');
    await expect(ssvNetwork.connect(account3)
      .withdraw('25000000000000'))
      .to.be.revertedWith('NotEnoughBalance');
  });
  ...
  it('Register another validator', async function () {
    ...
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9400000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(180000000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(420000000000);
    ...
    await progressBlocks(100);
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9298000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(10140600000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(561400000000);
  });
  ...
  it('Withdraw all when burn rate is positive', async function () {
    ...
    expect(await ssvNetwork.getAddressBurnRate(account2.address)).to.equal(400000000);
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9998000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(10000600000000);
  ...
  it('Liquidate when burn rate is positive', async function () {
    ...
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9998000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(10000600000000);
    await expect(ssvNetwork.connect(account1).liquidate([account1.address])).to.emit(ssvToken, 'Transfer').withArgs(ssvNetwork.address, account1.address, 9997000000000);
    expect(await ssvToken.balanceOf(account1.address)).to.equal(989997000000000);
    ...
    expect(await ssvNetwork.getAddressBurnRate(account2.address)).to.equal(700000000);
    ...
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(9996000000000);

    await progressBlocks(100)
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(9926000000000);
    ...
    await ssvNetwork.connect(account1).reactivateAccount('100000000000000');
    await expect(ssvNetwork.connect(account1).reactivateAccount(0)).to.be.revertedWith('AccountAlreadyEnabled');

    expect(await ssvNetwork.getAddressBurnRate(account1.address)).to.equal(2000000000);
    expect(await ssvNetwork.getAddressBurnRate(account2.address)).to.equal(100000000);

    await progressBlocks(50)
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(99898000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(9918800000000);
  });
  ...
  it('Withdraw all when burn rate is non-positive', async function () {
    ...
    await expect(ssvNetwork.connect(account3).withdrawAll()).to.emit(ssvToken, 'Transfer').withArgs(ssvNetwork.address, account3.address, 1100000000);
    ...
    expect(await ssvToken.balanceOf(account3.address)).to.equal(1100000000);
  });
  ...
  it('Remove a validator', async function () {
    ...
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9999000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(300000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(700000000);
  });
  ...
  it('Update operator fee', async function () {
    await ssvNetwork.connect(account3).declareOperatorFee(operatorsIds[3], "440000000");
    await ssvNetwork.connect(account3).executeOperatorFee(operatorsIds[3]);
    await progressBlocks(99);
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9895040000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(30300000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(74660000000);
  });
  ...
  it('Register a validator with deposit', async function () {
    ...
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9988000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(9996600000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(15400000000);
  });
  ...
  it('Remove a validator when overdue', async function () {
    await ssvNetwork.connect(account1).withdraw('2069000000000');
  ...
  it('Remove an operator', async function () {
    ...
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9998000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(10000600000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(1400000000);
    await progressBlocks(10)
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9988000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(9991600000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(20400000000);
    await ssvNetwork.connect(account2).removeValidator(validatorsPub[2]);
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9987000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(9990700000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(22300000000);
    await progressBlocks(10);
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9977000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(9993700000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(29300000000);
    await ssvNetwork.connect(account3).removeOperator(operatorsIds[4]);
    await progressBlocks(9);
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9967000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(9996700000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(36300000000);
  });

  it('Deactivate an operator', async function () {
    await progressBlocks(10)
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9990000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(3000000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(7000000000);
    await ssvNetwork.connect(account1).removeValidator(validatorsPub[0]);
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9989000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(3300000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(7700000000);
    await progressBlocks(10);
    expect(await ssvNetwork.getAddressBalance(account1.address)).to.equal(9989000000000);
    expect(await ssvNetwork.getAddressBalance(account2.address)).to.equal(3300000000);
    expect(await ssvNetwork.getAddressBalance(account3.address)).to.equal(7700000000);
  ...
  it('Operator max fee increase', async function () {
    ...
    await ssvNetwork.connect(account2).declareOperatorFee(operatorsIds[0], 110000000);
    await ssvNetwork.connect(account2).executeOperatorFee(operatorsIds[0]);
    expect(await ssvNetwork.getOperatorFee(operatorsIds[0])).to.equal(110000000);
    await ssvNetwork.updateOperatorFeeIncreaseLimit(2000);
    expect(await ssvNetwork.getOperatorFeeIncreaseLimit()).to.equal(2000);
    await expect(ssvNetwork.connect(account2).declareOperatorFee(operatorsIds[1], 25000000000)).to.be.revertedWith('FeeExceedsIncreaseLimit');
    await ssvNetwork.connect(account2).declareOperatorFee(operatorsIds[1], 240000000);
