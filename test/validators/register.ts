...
describe('Register Validator Tests', () => {
  ...
  it('Bulk register 2 validators with 4 operators into the same cluster', async () => {
    ...
    await expect(ssvNetworkContract.registerValidator(
      ...
      .withArgs(helpers.DB.owners[0].address,
        operatorIds,
        pks,
        shares,
        anyValue);
  ...
  it('Bulk register 4 validators with 4 operators into the same cluster', async () => {
    const pks = Array.from({ length: 4 }, (_, index) => helpers.DataGenerator.publicKey(index + 1));
    const operatorIds = [1, 2, 3, 4];
    const shares = Array.from({ length: 4 }, (_, __) => helpers.DataGenerator.shares(4));
    const depositAmount = minDepositAmount * 4;

    await helpers.DB.ssvToken.approve(ssvNetworkContract.address, depositAmount);
    await trackGas(ssvNetworkContract.registerValidator(
      pks,
      operatorIds,
      shares,
      depositAmount,
    ...
    ), [GasGroup.BULK_REGISTER_VALIDATOR_NEW_STATE]);
  });
  ...
  it('Bulk register 4 validators with 7 operators into the same cluster', async () => {
    const pks = Array.from({ length: 4 }, (_, index) => helpers.DataGenerator.publicKey(index + 1));
    ...
    ), [GasGroup.BULK_REGISTER_VALIDATOR_NEW_STATE_7]);
  });
  ...
  it('Bulk register 4 validators with 13 operators into the same cluster', async () => {
    const pks = Array.from({ length: 4 }, (_, index) => helpers.DataGenerator.publicKey(index + 1));
    ...
    ), [GasGroup.BULK_REGISTER_VALIDATOR_NEW_STATE_13]);
  });
  ...
  it('Surpassing max number of validators per operator reverts "ExceedValidatorLimit"', async () => {
    helpers.registerValidators(2, 50, (minDepositAmount * 50).toString(), [8, 9, 10, 11]);
  ...
  it('Bulk register validator with public keys and shares mismatched length reverts "PublicKeysSharesLengthMismatch"', async () => {
    await expect(ssvNetworkContract.connect(helpers.DB.owners[1]).registerValidator(
      [helpers.DataGenerator.publicKey(2), helpers.DataGenerator.publicKey(3)],
      [1, 2, 3, 4],
      [helpers.DataGenerator.shares(4)],
      minDepositAmount,
      {
    ...
    )).to.be.revertedWithCustomError(ssvNetworkContract, 'PublicKeysSharesLengthMismatch');
  });
});
