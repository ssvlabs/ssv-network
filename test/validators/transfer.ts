...
describe('Transfer Validator Tests', () => {
  ...
  it('Transfer validator to a cluster with 7 operators', async () => {
    ...
    await helpers.transferValidator(4, clusterResult1.validators[0].publicKey, helpers.DataGenerator.cluster.byId(clusterId), minDepositAmount, [GasGroup.TRANSFER_VALIDATOR_NON_EXISTING_POD]);
  });
  ...
  it('Transfer validator to an existing cluster', async () => {
    ...
    // Transfer validator
    const transfredValidator1 = await helpers.transferValidator(4, clusterResult1.validators[0].publicKey, helpers.DataGenerator.cluster.byId(clusterResult3.clusterId), minDepositAmount, [GasGroup.TRANSFER_VALIDATOR_NON_EXISTING_POD]);
